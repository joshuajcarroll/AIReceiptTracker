...
const saveToDatabaseTool = createTool({
    ...
    parameters: z.object({
        ...
        .describe(
            "The readable display name of the receipt to show in the UI. If the file name is not human readable, use this to give a more readanle name",
        ),
        ...
        .describe(`A summary of the receipt, including the merchant name, address, contact, transaction date, transaction amount, and currency. Include a human readable summary of the receipt.
            Mention both invoice and receipt number if both are present. Include some key details about the items on the receipt, this is a special featured
            summary so it should include some key details about the items on the receipt with some context.`,
        ),
        currency: z.string(),
        items: z.array(
            z.object({
    ...
    handler: async (params, context ) => {
        ...
        const result = await context.step?.run(
            ...
            async () => {
                try {
                    //Call the Convex mutation to update the receipt with the extraced data
...
export const databaseAgent = createAgent({
    ...
    system:
    "You are a helpful assistant that takes key information regarding receipts and saves it to the conves database.",
    model: openai({
        model:"gpt-4o-mini",
        defaultParameters: {
